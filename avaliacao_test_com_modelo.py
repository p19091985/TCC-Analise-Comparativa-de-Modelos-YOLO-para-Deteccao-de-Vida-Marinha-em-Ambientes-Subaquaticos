...
CONFIG = {
    "DIRETORIO_RUNS": "runs/detect",
    "DIRETORIO_DATASETS": "dataset_descompactado",
...
class ValidadorAbsoluto:
    ...
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.diretorio_raiz = Path(__file__).parent.resolve()
        self.diretorio_runs = self.diretorio_raiz / self.config["DIRETORIO_RUNS"]
        self.diretorio_datasets = self.diretorio_raiz / self.config["DIRETORIO_DATASETS"]
        self.timestamp_utc = datetime.datetime.now(datetime.timezone.utc).isoformat()
        self.logger = self._inicializar_logger()
        self.artefato_final = self._criar_estrutura_artefato()

    def _inicializar_logger(self) -> logging.Logger:
        timestamp_log = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        nome_arquivo_log = self.diretorio_raiz / f"log_validacao_absoluta_{timestamp_log}.log"
        logging.basicConfig(
            level=logging.INFO,
        ...
        return logging.getLogger('ValidadorAbsolutoLogger')
    ...
    def _extrair_nome_dataset(self, nome_diretorio_run: str) -> Optional[str]:
        """
        Extrai o nome do dataset do nome do diretório do run de forma robusta.
        Funciona para nomes de dataset que contêm underscores.
        Ex: ..._on_aquarium_pretrain_21-09-2025_15-37-21 -> aquarium_pretrain
        """
        try:
            # Pega a parte da string depois do "_on_"
            parte_depois_on = nome_diretorio_run.split('_on_')[1]

            # Separa a string a partir da DIREITA, no máximo 2 vezes.
            # Isso isola o nome do dataset da data e da hora.
            # Ex: "aquarium_pretrain_21-09-2025_15-37-21" -> ['aquarium_pretrain', '21-09-2025', '15-37-21']
            ...
            self.logger.warning(
                ...
                f"Padrão esperado '_on_NOMEDATASET_DATA_HORA' não encontrado.")
            return None
        except Exception:
    ...
    def _carregar_detalhes_dataset(self, nome_dataset: str) -> Optional[Dict[str, Any]]:
        caminho_yaml = self.diretorio_datasets / nome_dataset / 'data.yaml'
        if not caminho_yaml.is_file():
            self.logger.error(
                f"Arquivo de configuração '{caminho_yaml}' para o dataset '{nome_dataset}' não foi encontrado.")
            return None
        try:
            with open(caminho_yaml, 'r', encoding='utf-8') as f:
                config_dataset = yaml.safe_load(f)
                config_dataset['caminho_yaml_absoluto'] = str(caminho_yaml)
                return config_dataset
        except yaml.YAMLError:
            self.logger.exception(f"Falha ao processar o arquivo YAML '{caminho_yaml}'.")
            return None
    ...
    def _executar_validacao_para_candidato(self, candidato: Dict[str, Path]):
        ...
        if not nome_dataset:
            self._registrar_falha(run_dir.name, str(model_path),
                                  "Não foi possível determinar o dataset a partir do nome do diretório.")
            return

        detalhes_dataset = self._carregar_detalhes_dataset(nome_dataset)
        if not detalhes_dataset:
            self._registrar_falha(run_dir.name, str(model_path),
                                  f"Falha ao carregar a configuração do dataset '{nome_dataset}'.")
            return

        resultado = {
            "status": "FALHA",
            "nome_run": run_dir.name,
            "caminho_modelo": str(model_path),
            "dataset": {
                "nome_identificado": nome_dataset,
        ...
        try:
            ...
            metricas = modelo.val(data=detalhes_dataset['caminho_yaml_absoluto'], split='test', device=dispositivo,
                                  verbose=False)
    ...
    def _salvar_artefato(self):
        """
        Salva os resultados da validação em um arquivo .txt formatado como CSV.
        """
        nome_arquivo = f"{self.config['NOME_BASE_RELATORIO']}_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.txt"
        caminho_arquivo = self.diretorio_raiz / nome_arquivo
        self.logger.info(f"Salvando relatório em formato TXT (CSV) em: {caminho_arquivo}")

        DELIMITADOR = ";"

        cabecalho = [
            "nome_run", "status", "dataset_nome", "mAP50_95", "mAP50", "mAP75",
            "precisao", "recall", "velocidade_preprocess_ms", "velocidade_inference_ms",
            "velocidade_postprocess_ms", "mensagem_erro"
        ]
...
if __name__ == "__main__":
    validador = ValidadorAbsoluto(CONFIG)
    validador.executar()