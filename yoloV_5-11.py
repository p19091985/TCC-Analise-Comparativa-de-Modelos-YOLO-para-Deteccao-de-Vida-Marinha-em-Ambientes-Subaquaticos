...
CONFIG = {
    ...
    "TRAINING_JOBS": [
       {'modelo': 'YOLOv5s', 'base_model': 'yolov5s.pt'},
        ...
        {'modelo': 'YOLO11s', 'base_model': 'yolo11s.pt'},
        {'modelo': 'YOLO11m', 'base_model': 'yolo11m.pt'},
    ],
...
class PipelineTreinamento:
    ...
    def _verificar_ambiente(self) -> str:
        ...
        for job in self.config["TRAINING_JOBS"]:
            try:
                YOLO(job['base_model'])  # Tenta carregar para verificar
    ...
    def _medir_latencia(self, model_path: str, device: str) -> float:
        ...
        try:
            ...
            model.to(device)
            dummy_input = torch.randn(1, 3, self.config['IMG_SIZE'], self.config['IMG_SIZE']).to(device)
    ...
    def _executar_job(self, job: Dict[str, Any], dataset_name: str, device: str):
        ...
        try:
            ...
            resultado_job.update({
                ...
                "mAP50": final_metrics.get('metrics/mAP50(B)', 0),  # <<< NOVA MÉTRICA
                "Precision": precision,
                "Recall": recall,
                "F1_Score": f1_score,
                "Latency_ms": latency,
    ...
    def _gerar_relatorio(self):
        ...
        try:
            with open(report_path, 'w', newline='', encoding='utf-8') as f:
                ...
                writer.writeheader()
                # Formata os números de ponto flutuante para 4 casas decimais
